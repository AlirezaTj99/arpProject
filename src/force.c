...
int main(){
    ...
    while(TRUE)
    {
        ...
        sprintf(str3, format_string3, ee_x, ee_y, vx, vy);
        write(fd3, str3, strlen(str3)+1);
        close(fd3);

        fd4 = open(myfifo4,O_WRONLY);               // Send the drone position to input processor
        if (fd4 == -1) {
            perror("Error opening FIFO4");
        }
        sprintf(str4, format_string4, ee_x, ee_y);
        ...
        read(fd8, str8, 80);
        sscanf(str8, format_string8, &ox1, &ox2, &ox3, &ox4, &ox5, &ox6, &ox7, &ox8, &ox9, &ox10, &oy1, &oy2, &oy3, &oy4, &oy5, &oy6, &oy7, &oy8, &oy9, &oy10);
        close(fd8);

        fd9 = open(myfifo9,O_RDONLY);               // Import the goals position
        if (fd9 == -1) {
            perror("Error opening FIFO9");
        }
        read(fd9, str9, 80);
        sscanf(str9, format_string9, &gx1, &gx2, &gx3, &gx4, &gx5, &gx6, &gx7, &gx8, &gx9, &gx10, &gy1, &gy2, &gy3, &gy4, &gy5, &gy6, &gy7, &gy8, &gy9, &gy10);
        close(fd9);
        ...
        fd11 = open(myfifo11,O_WRONLY);                     // Let every bodey know if all the goals been reched
        if (fd11 == -1) {
            perror("Error opening FIFO11");
        }
        sprintf(str11, format_string11, reachedAllTheGoals);
        write(fd11, str11, strlen(str11)+1);
        close(fd11);

        fd12 = open(myfifo12,O_WRONLY);                     // Let every bodey know if all the goals been reched
        if (fd12 == -1) {
            perror("Error opening FIFO12");
        }
        sprintf(str12, format_string12, reachedAllTheGoals);
        write(fd12, str12, strlen(str12)+1);
        close(fd12);
        usleep(1700);
